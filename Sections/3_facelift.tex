...
each of the scenes in our augmented set plus the synthetic image $\hat{I_j}$ as a 4096 dimensional feature vector derived from the FC7 layer of the PlacesNet~\cite{zhou2014learning}; \emph{ii)} computing the distance (as $L_2$ Norm) between $\hat{I_j}$'s feature vector and each  original scene's feature vector; and \emph{iii)} selecting the  scene  in our augmented set most similar (smaller distance) to $\hat{I_j}$. This results into the selection of the beautified scene $I_j$.
