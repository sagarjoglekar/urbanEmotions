...
\begin{table*}[h]
	...
	\resizebox{\linewidth}{!}{%
		\begin{tabular}{|c|p{10cm}|}
			...
			\hline		
			Landmarks & Losing a bearing in the city is not a very pleasant experience. Hence presence of recognisable and  guiding landmarks influences the perception of an urban space \cite{lynch1960image,quercia2014aesthetic,ewing2013measuring}.\\
			\hline
...
\label{eq:entropy} 
\end{equation}
where $i$ is the $i^{th}$ Segnet's label. The total number of labels is twelve. The higher $H(X)$, the  higher the scene's entropy, that is, the higher the scene's complexity. It has been proposed, that the perception of aesthetics or pleasantness follows an `inverted U' shape\cite{ulrich1983aesthetic}. To test hypothesis \emph{H4}, we show the percentage of scenes that fall into a complexity bin  (Figure~\ref{fig:complexity}): beautified scenes are of low to medium complexity, while uglified ones are of high complexity.
